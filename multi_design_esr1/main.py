...
def main(args, output_dir):
    ...
    criterion = nn.NLLLoss(reduction='sum')  # TODO: use pad or just eos?
    model.to(device)
    criterion.to(device)
    model.train()
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            x, ba, sas, qed = x.to(device), ba.to(device), sas.to(device), qed.to(device)
            ...
            z_samples, z_grads = model.infer_z(z=z_0_posterior, x=x, x_len=x_len, y=[ba, sas, qed], beta=args.beta,
        ...
        writer, prior_prop_valid = evaluation(args, epoch, test_loader, model, logger, writer)
