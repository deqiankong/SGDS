...
def single_prop_design(model_path, args):
    ...
    for epoch in range(num_design):
        if epoch == 0:
            design_data = DesignDataset(x=test_data.Xdata, y=test_data.ba0)
        else:
        ...
        for iter in range(num_iters):
            for i, data in enumerate(tqdm(train_loader, desc='Training')):
                ...
                # print('MLP error: ', mlp_error.item())
                # generator update
                optimizer.zero_grad()
        ...
        for i, data in enumerate(tqdm(design_loader, desc='Sampling')):
            ...
            z_y, _ = model.infer_z_given_y(z_0, y + 1, n_iter=20, step_size=0.5)
        ...
        # y_new = torch.zeros(x_new.shape[0])
